...
class VNPayConfig
{
    ...
    public static function fromEnvironment(array $env): self
    {
        ...
        $config->returnUrl = trim($env['VNPAY_RETURN_URL'] ?? 'http://localhost:8080/StygianBlue/vnpay_php/vnpay_return.php');
        $config->ipnUrl = isset($env['VNPAY_IPN_URL']) ? trim($env['VNPAY_IPN_URL']) : null;
