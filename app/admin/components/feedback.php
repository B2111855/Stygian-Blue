...
$tableCheckResult = $conn->query("SHOW TABLES LIKE 'phan_hoi_cua_khach_hang'");
if (!$tableCheckResult || $tableCheckResult->num_rows === 0) {
  die('<div style="padding: 20px; background: #fee; color: #c00; border-radius: 5px;">
    <strong>Lỗi:</strong> Bảng phản hồi không tồn tại trong cơ sở dữ liệu.
  </div>');
}
...
if (!$totalQuery) {
  die('<div style="padding: 20px; background: #fee; color: #c00; border-radius: 5px;">
    <strong>Lỗi truy vấn:</strong> ' . htmlspecialchars(mysqli_error($conn)) . '
  </div>');
}
...
// ============ OPTIMIZED QUERY (Using ROW_NUMBER alternative) ============
$query = "
  SELECT 
    ROW_NUMBER() OVER (ORDER BY ph.NGAY_GUI DESC) as ID_PHAN_HOI,
    ph.NOI_DUNG,
    ph.XEP_HANG_DV,
    ph.NGAY_GUI,
    ph.ID_TK,
  ...
  GROUP BY ph.ID_TK, ph.ID_DV, ph.NOI_DUNG, ph.NGAY_GUI, ph.XEP_HANG_DV, 
           tk.HO_TEN, tk.SDT, dv.TEN_DV, lh.ID_LICHHEN, lh.THOI_GIAN_BAT_DAU, lh.DIA_CHI_HEN
...
if (!$result) {
  // Fallback to simpler query without GROUP_CONCAT if ROW_NUMBER fails
  $query = "
    SELECT 
      1 as ID_PHAN_HOI,
      ph.NOI_DUNG,
      ...
      ?>
        <div class="fb-card bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-md transition hover:-translate-y-1"
          ...
          <div class="p-4 space-y-3">
            ...
            <div class="bg-slate-50 rounded-lg p-2">
              <p class="text-xs text-slate-700">
                ...
                <strong><?= htmlspecialchars($fb['service_name'] ?? 'N/A') ?></strong>
              </p>
            </div>

            <!-- Appointment info (if exists) -->
            <?php if ($fb['ID_LICHHEN']): ?>
              <div class="bg-sky-50 rounded-lg p-2 border border-sky-200 text-xs text-sky-900 space-y-1">
                <p class="font-semibold"><i class="fas fa-calendar-check mr-1"></i>Lịch hẹn</p>
                <p><strong>📅</strong> <?= date('d/m/Y H:i', strtotime($fb['appointment_start'])) ?></p>
                <p><strong>📍</strong> <?= htmlspecialchars($fb['appointment_location'] ?? 'N/A') ?></p>
                <?php if ($fb['assigned_staff']): ?>
                  <p><strong>👤</strong> <?= htmlspecialchars($fb['assigned_staff']) ?></p>
                <?php endif; ?>
              </div>
            <?php else: ?>
              <div class="bg-slate-50 rounded-lg p-2 text-xs text-slate-600">
                <i class="fas fa-info-circle mr-1"></i>Chưa có lịch hẹn
              </div>
            <?php endif; ?>
          ...
          <div class="p-3 bg-slate-50 border-t border-slate-100 flex gap-2 flex-wrap">
            ...
            <a href="?page=services&edit=<?= $fb['ID_DV'] ?>" 
