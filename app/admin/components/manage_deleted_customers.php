<?php
// No need to load header since it's included in admin_dashboard
?>
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center mb-4">
            <div>
                <h1 class="text-4xl font-bold text-gray-800 mb-2">
                    <i class="fas fa-trash-alt text-red-500 mr-3"></i>Khách Hàng Đã Xóa
                </h1>
                <p class="text-gray-600 text-sm">
                    <i class="fas fa-info-circle mr-2"></i>Xem danh sách khách hàng đã bị xóa mềm. Có thể hoàn tác trong vòng 30 ngày.
                </p>
            </div>
            <a href="admin_dashboard.php?page=customers" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-semibold transition-colors duration-200">
        ...
        <div class="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 rounded-lg p-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-gray-600">Tổng khách hàng đã xóa</p>
            ...
            <table id="customerTable" class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-700">Số Điện Thoại</th>
    ...
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <div class="bg-gradient-to-r from-green-50 to-blue-50 border-b-2 border-gray-100 px-6 py-4 flex justify-between items-start">
            <h5 class="text-xl font-bold text-gray-800">
                <i class="fas fa-undo-alt text-green-600 mr-2"></i>Hoàn Tác Xóa Khách Hàng
            </h5>
        ...
        <div class="p-6">
            ...
            <div class="p-4 bg-green-50 border-l-4 border-green-500 rounded flex items-start gap-3">
                ...
                <div>
                    <p class="font-semibold text-gray-800">Khách hàng sẽ được khôi phục</p>
                    <p class="text-sm text-gray-600 mt-1">Trạng thái sẽ được đặt lại thành ACTIVE và có thể đặt lịch hẹn ngay lập tức.</p>
                </div>
            </div>
        </div>
...
<script>
    ...
    function openRestoreModal(id, name) {
        ...
        document.getElementById('restoreMessage').innerHTML = 
            `Bạn có chắc muốn khôi phục khách hàng <strong>${name}</strong> không?`;
    ...
    function loadDeletedCustomers(page = 1, search = '') {
        ...
        fetch(`api/api_customers.php?${params}`, {
            credentials: 'include'
        })
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('tableBody').innerHTML = 
                        '<tr class="text-center"><td colspan="6" class="text-muted">Không có dữ liệu</td></tr>';
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showAlert('danger', 'Lỗi khi lấy dữ liệu');
            });
    }
    ...
    function renderTable(customers) {
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center gap-3">
                            ...
                            <p class="text-gray-500 font-semibold">Không có khách hàng nào đã xóa</p>
                            <p class="text-gray-400 text-sm">Tất cả khách hàng đang hoạt động</p>
                        </div>
                    </td>
                </tr>
            `;
        ...
        document.getElementById('deletedCount').textContent = customers.length;

        tableBody.innerHTML = customers.map(cust => {
            const deletedDate = new Date(cust.DELETED_AT).toLocaleDateString('vi-VN', { 
                year: 'numeric', 
                month: '2-digit', 
                day: '2-digit',
                hour: '2-digit',
            ...
            return `
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                    <td class="px-6 py-4">
                        <span class="inline-block px-3 py-1 bg-gray-100 text-gray-800 rounded-lg text-sm font-semibold">
                            ${cust.ID_TK}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="font-semibold text-gray-800">${cust.HO_TEN}</span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-gray-600 text-sm">${cust.EMAIL}</span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-gray-600 text-sm">${cust.SDT}</span>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-center">
                        <button onclick="openRestoreModal('${cust.ID_TK}', '${cust.HO_TEN.replace(/'/g, "\\'")}' )" class="inline-flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg font-semibold transition-colors duration-200 text-sm">
    ...
    function renderPagination(pagination) {
        ...
        prevBtn.innerHTML = `
            <a class="px-4 py-2 rounded-lg inline-flex items-center gap-2 font-semibold transition-colors" href="#" onclick="event.preventDefault(); ${pagination.page > 1 ? `loadDeletedCustomers(${pagination.page - 1}, '${document.getElementById('searchInput').value}')` : ''}" ${pagination.page === 1 ? 'disabled' : ''}>
        ...
        nextBtn.innerHTML = `
            <a class="px-4 py-2 rounded-lg inline-flex items-center gap-2 font-semibold transition-colors" href="#" onclick="event.preventDefault(); ${pagination.page < pagination.pages ? `loadDeletedCustomers(${pagination.page + 1}, '${document.getElementById('searchInput').value}')` : ''}" ${pagination.page === pagination.pages ? 'disabled' : ''}>
                <span>Tiếp</span>
    ...
    document.getElementById('confirmRestoreBtn').addEventListener('click', () => {
        ...
        formData.append('ID_TK', selectedCustomerId);
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    loadDeletedCustomers(currentPage, document.getElementById('searchInput').value);
                }, 1000);
            } else {
                showAlert('danger', data.message);
            }
