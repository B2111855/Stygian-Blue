...
function getEmployeesSearch($search = "", $limit = 6, $offset = 0)
{
    global $conn;
    $query = "
        SELECT nv.ID_TK, nv.ID_CN, nv.CHUYEN_MON, tk.HO_TEN, tk.NGAY_SINH, tk.DIA_CHI, tk.EMAIL, tk.SDT, cn.TEN_CN
        FROM nhan_vien nv
...
if (isset($_POST['edit_employee'])) {
    ...
    if (empty($ho_ten) || empty($ngay_sinh) || empty($dia_chi) || empty($email) || empty($sdt) || empty($id_cn) || empty($chuyen_mon)) {
        ...
        if (!isset($errorMessage) && mysqli_query($conn, $updateTaiKhoanQuery)) {
            $updateNhanVienQuery = "UPDATE nhan_vien SET ID_CN = '$id_cn', CHUYEN_MON = '$chuyen_mon' WHERE ID_TK = '$id_tk'";
...
function handleAddEmployee($data)
{
    ...
    $insertNhanVien = $conn->prepare("INSERT INTO nhan_vien (ID_TK, ID_CN, CHUYEN_MON) VALUES (?, ?, ?)");
    $insertNhanVien->bind_param("sss", $id_tk, $id_cn, $chuyen_mon);
...
if (isset($_GET['edit'])) {
    $id_tk = $_GET['edit'];
    $editQuery = "SELECT nv.ID_TK, nv.ID_CN, nv.CHUYEN_MON, tk.HO_TEN, tk.NGAY_SINH, tk.DIA_CHI, tk.EMAIL, tk.SDT, tk.MAT_KHAU
              FROM nhan_vien nv
              INNER JOIN tai_khoan tk ON nv.ID_TK = tk.ID_TK
