...
if ($source === 'manager' || strpos($referer, 'manager_dashboard.php') !== false) {
...
function calculateTotalPrice($idLichHen)
{
    ...
    // Fallback nếu không có line dịch vụ trong BOOKING_ITEM
    if ($totalDV <= 0) {
        $queryDV = "SELECT 
               (SELECT DON_GIA 
    ...
    if ($colTF = mysqli_query($conn, "SHOW COLUMNS FROM lich_hen LIKE 'TRAVEL_FEE'")) {
        if (mysqli_num_rows($colTF) > 0) {
        ...
        mysqli_free_result($colTF);
    }

    return [
        'totalDV' => $totalDV,
        'totalTB' => $totalTB,
        'total' => $totalDV + $totalTB + $travelFee
    ];
}
...
$rentalDays = 1;
if ($isRental) {
    $start = !empty($invoice['NGAY_NHAN']) ? strtotime($invoice['NGAY_NHAN']) : null;
    $end = !empty($invoice['NGAY_TRA_DK']) ? strtotime($invoice['NGAY_TRA_DK']) : null;
    if ($start && $end && $end > $start) {
        $rentalDays = max(1, (int)ceil(($end - $start) / 86400));
    }
}
...
if ($isRental) {
    $riStmt = mysqli_prepare($conn, "
        SELECT ct.SO_LUONG, ct.DON_GIA_AP_DUNG, tp.TEN, tp.GIA_THUE AS GIA_THUE_BASE
...
<head>
    ...
    <style>
        @media print {
            ...
            .invoice-container { max-width: 100% !important; box-shadow: none !important; border-radius: 0; margin: 0; padding: 20mm; }
            /* Hiển thị riêng phần hóa đơn khi in, dùng được cả khi trang này được include */
        ...
        <div class="mb-8">
            ...
            <table class="w-full border-collapse text-sm">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr class="border">
                            <td class="p-3"><?= htmlspecialchars($invoice['TEN_DV']) ?></td>
                            <td class="p-3 text-right"><?= number_format((float)$invoice['GIA_DV'], 0, ',', '.') ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
            ...
            <?php if (!empty($rentalItems)): ?>
                <table class="w-full border-collapse text-sm">
                    <thead>
                        <tr class="bg-gray-100 border">
                            ...
                            <th class="p-3 text-center">Số ngày thuê</th>
                            <th class="p-3 text-right">Đơn giá / ngày</th>
                            <th class="p-3 text-right">Thành tiền</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $rentalSubtotal = 0; $discountTotal = 0; foreach ($rentalItems as $ri):
                            $qty = (int)$ri['SO_LUONG'];
                            $priceApplied = (int)$ri['DON_GIA_AP_DUNG'];
                            $basePrice = isset($ri['GIA_THUE_BASE']) ? (int)$ri['GIA_THUE_BASE'] : $priceApplied;
                            $sub = $qty * (int)$rentalDays * $priceApplied; $rentalSubtotal += $sub;
                            $discPerDay = max(0, $basePrice - $priceApplied);
                            $discountTotal += $qty * (int)$rentalDays * $discPerDay;
                        ?>
                            <tr class="border">
                                <td class="p-3"><?= htmlspecialchars($ri['TEN']) ?></td>
                                <td class="p-3 text-center"><?= (int)$rentalDays ?></td>
                                <td class="p-3 text-right"><?= number_format($ri['DON_GIA_AP_DUNG'], 0, ',', '.') ?></td>
                                <td class="p-3 text-right"><?= number_format($sub, 0, ',', '.') ?></td>
                            </tr>
                        <?php endforeach; ?>
                        <?php // Fallback: if per-item discount not available, derive from invoice total
                        if ($discountTotal <= 0) {
                            $discountTotal = max(0, (int)$rentalSubtotal - (int)$invoice['TONG_TIEN']);
                        }
                        ?>
                    </tbody>
                </table>
                <div class="mt-4 text-sm">
                    <div class="flex justify-between">
                        <span class="text-gray-700">Số ngày thuê</span>
                        <span class="font-semibold text-gray-900"><?= (int)$rentalDays ?> ngày</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-700">Khuyến mãi</span>
                        <span class="font-semibold text-gray-900"><?= number_format((int)$discountTotal, 0, ',', '.') ?> VND</span>
                    </div>
                </div>
            <?php endif; ?>
        </div>
